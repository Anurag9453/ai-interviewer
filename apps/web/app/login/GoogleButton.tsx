...
/**
 * Client component: signInWithOAuth must run in the browser — it navigates
 * the whole page to Google, which a server action cannot do.
 */
  ...
  async function onClick() {
    ...
    const { error } = await supabase.auth.signInWithOAuth({
      ...
      options: {
        ...
        // Without this, Google silently reuses whichever of the browser's
        // signed-in accounts is currently active instead of prompting —
        // real friction for anyone with more than one Google account, not
        // just a dev/testing inconvenience.
        queryParams: { prompt: "select_account" },
      },
    });
    // On success the browser is already navigating to Google — this only
    // runs if signInWithOAuth itself failed before redirecting.
